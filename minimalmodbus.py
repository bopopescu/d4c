...
class Instrument():
    ...
    def __init__(self, port, slaveaddress, BAUDRATE = 19200, PARITY   = serial.PARITY_EVEN, BYTESIZE = 8, \
...
def _extractPayload(response, slaveaddress, functioncode):
    ...
    if receivedFunctioncode == _setBitOn(functioncode, BITNUMBER_FUNCTIONCODE_ERRORINDICATION):
        raise ValueError('The slave is indicating an error.') # The response is: {0!r}'.format(response)) # neeme
