#!/usr/bin/env python
## Neeme Takis added speed control via default variables 24.8.2015
#
...
.. moduleauthor:: Jonas Berg <pyhys@users.sourceforge.net> # modified by neeme
...
#PARITY   = serial.PARITY_EVEN #  NONE # neeme
...
#TIMEOUT  = 0.3   # 0.05 # neeme
"""Default value for the timeout value in seconds (float)."""

#CLOSE_PORT_AFTER_EACH_CALL = True #  False # neeme
...
class Instrument():
    ...
    def __init__(self, port, subordinateaddress, BAUDRATE = 19200, PARITY   = serial.PARITY_EVEN, BYTESIZE = 8, \
        STOPBITS = 1, TIMEOUT  = 0.3, CLOSE_PORT_AFTER_EACH_CALL = True):
    ...
    def write_register(self, registeraddress, value, numberOfDecimals=0, functioncode=6, signed=False): # neeme
    ...
    def _genericCommand(self, functioncode, registeraddress, value=None, \
        ...
        if functioncode == 6 and payloadformat == PAYLOADFORMAT_REGISTER and numberOfRegisters != 1: # neeme
    ...
    def _communicate(self, message):
        ...
        if sys.version_info[0] > 2:
            ...
            messagehex =  str(base64.b16encode( message ))[2:][:-1]

        if self.debug:
            messagehex=repr(message) # comment for py3?
            #_print_out( 'MinimalModbus debug mode. Writing to instrument: ' + repr(message) )
            _print_out( '=> ' + messagehex ) # neeme
        ...
        if sys.version_info[0] > 2:
            #str(base64.b16encode(a))[2:][:-1]
            answerhex = str(base64.b16encode( answer ))[2:][:-1]  # Convert to (hex) string, neeme
        ...
        if self.debug:
            _print_out( '<= ' +  answerhex )  # neeme py3
            # _print_out( '<= ' + repr(answer) )  # py2
...
def _extractPayload(response, subordinateaddress, functioncode):
    ...
    if receivedFunctioncode == _setBitOn(functioncode, BITNUMBER_FUNCTIONCODE_ERRORINDICATION):
        raise ValueError('The subordinate is indicating an error.') # The response is: {0!r}'.format(response)) # neeme
