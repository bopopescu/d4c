# airmonitor.py for airmonitor_srik  started 15.01.2014 ###### SQL TABLES in memory!!! feed to webserver via indata()
...
APVER='channelmonitor_pm.py srik 18.01.2014'  # linux and python3 -compatible. # THIS IS FROM droid/channelmonitor/airmonitor_srik branch!
...
# made a branch airmonitor_srik. using sql files on flash in order to make sql2json.py working
# 18.01.2014 variables to aichannels, dichannels, setup. dsp_id instead of comment.
# 19.01.2014 starting to integrate webserver.py (in airmonitor branch), variables feed via indata 
...
def read_proxy(what): # read modbus proxy registers, wlan mac most importantly. start only if tcp conn already exists! parameter 'all' or anything
    ...
    try:
        ...
        if WLANoldip != WLANip:
            ...
            syslog(msg) # debug
            if OSTYPE == 'android':
                droid.ttsSpeak('wireless ip has changed to '+WLANip) 
            # change also SIP value in setup table!
            if change_setup('SIP',WLANip) == 0: # successful change in memory, not in file yet!
                setup_change = 1 # flag the change
                msg='SIP in setup changed to '+WLANip
                ...
                msg='failed to change register SIP in setup!'
            print(msg)
            syslog(msg)                
        ...
        syslog(msg) # debug
        if OSTYPE == 'android':
            while (mac[0:3] != 'D05' and mac[0:3] != 'BCF'): # invalid mac after reboot if wlan was off before reboot
    ...
    try:
        ...
        for row in cursor3: # services
            ...
            if make_aichannel_svc(val_reg,sta_reg) == 0: # successful svc insertion into buff2server
                pass
                #print('tried to report svc',val_reg,sta_reg) # debug
            else:
...
def make_dichannels(): # di services into to-be-sent buffer table BUT only when member(s) changed or for renotification AND updated less than 5 s ago
    ...
    try:
        ...
        for row in cursor3: # services to be processed. either just changed or to be resent
            ...
            if chg == 1: # message due to dichannel state change
                msg='DI service to be reported due to change: '+val_reg
            else:
                msg='DI service '+val_reg+' to be REreported, last reporting was '+str(ts-ts_last)+' s ago, at '+str(ts_last) # , ts now=',ts
            print(msg)
            syslog(msg)
        ...
        conn3.commit() # dichannels transaction end, also for make_dichannel_svc()
...
def make_dichannel_svc(val_reg,sta_reg,chg):    # ONE service compiling and buffering into buff2server if ok
    ...
    for srow in cursor3a: # ridu tuleb nii palju kui selle teenuse liikmeid, pole oluline milliste mba ja readd vahele jaotatud
        ...
        #    return 1  # stale data, not to be sent! # N 20.01.2014 do not break here...
...
def get_svc(table,service,member = 1): # send values to display on webui
    status=None
    value=None
    msg=''
    Cmd3="select status,value from "+table+" where val_reg='"+service+"' and member='"+str(member)+"'" # loeme teenuseliikme info, peaks ka ts kontrollima
    try:
        cursor3.execute(Cmd3)  
        conn3.commit()
        
        for row in cursor3: # should be just one row
            #print(repr(row)) # debug
            #syslog(repr(row)) # debug
            status=int(float(row[0]))
            value=int(float(row[1].split(' ')[member - 1]))  # multiple space-delimited values inside this value are possible!
            #ts=int(float(row[2])) # different ts name in aichannels vs dichannes
            #msg='get_svc sta val for '+service+':'+str(status)+' '+str(value)
            #print(msg) # debug
            #syslog(msg) 
            
    except:
        msg='get_svc select problem'
        print(msg) # debug
        syslog(msg) # debug
        traceback.print_exc()
...
def svc2indata(): # report values needed for indata in case of srik airmonitor
    svc=['MOS','WLS','USS','R1S','T1W','AHW','AQW']
    key=[100,101,102,8,600,2,3]
    for i in range(len(svc)):
        if svc[i][-1] == 'S': # status, di
            statusvalue=get_svc('dichannels',svc[i])
            typename='DI'
        else: # ai,ti
            statusvalue=get_svc('aichannels',svc[i])
            typename='AI'
        status=statusvalue[0]
        value=statusvalue[1]
        if status != None:
            indata.write(typename+str(key[i]), {"value" : value, "status" : status }) # indata.write(svc, status, value) 
        else:
            syslog('check channel data - got no status for svc '+svc[i])
            print('check channel data - got no status for svc '+svc[i])
    msg='indata is '+str(indata) # debug 
    print(msg) # debug
    syslog(msg) # debug
...
try:
    from droidcontroller.indata import InData # feeds data for webserver
    indata = InData()
    #from droidcontroller.indata_pacui import InDataPacui # this formats json
        
    from droidcontroller.webserver import WebServer # servers http port 8080 
    ...
    traceback.print_exc()
    msg='problems with import from sub droidcontroller'
    print(msg)
    syslog(msg)
...
UDPSock.settimeout(1) # (0.1) use 1 or more for testing # in () timeout to wait for data from server. defines also main loop interval / execution speed!!
...
while stop == 0: # ################  MAIN LOOP BEGIN  ############################################################
    ...
    if ts>appdelay+ts_lastappmain:  # time to read analogue registers and counters, not too often
        ...
        svc2indata() # send some data to webui, interval about 5 sec
